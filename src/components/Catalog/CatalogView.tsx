...
export const CatalogView: React.FC = () => {
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    if (formData.category?.trim()) {
      await addCategory(formData.category.trim());
    }
      ...
      <div className="bg-stone-900 border border-stone-800 rounded-2xl p-4 flex flex-col gap-4">
        ...
        <div className="border-t border-stone-800/60 pt-3">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center space-x-2 space-x-reverse text-stone-400 text-xs font-bold">
            ...
            {canAdd && (
              <button
                type="button"
                onClick={async () => {
                  const catName = window.prompt('أدخل اسم القسم الجديد لإضافته للأقسام:');
                  if (!catName || !catName.trim()) return;
                  const clean = catName.trim();
                  await addCategory(clean);
                  setSelectedCategory(clean);
                }}
                className="text-[11px] font-bold text-amber-500 hover:text-amber-400 flex items-center gap-1 hover:underline transition-colors"
          ...
          <div className="bg-stone-900 border border-stone-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSaveProduct} className="p-4 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="relative">
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-xs font-bold text-stone-300">القسم / التصنيف</label>
                  <button
                    type="button"
                    onClick={async () => {
                      const newCat = window.prompt('أدخل اسم قسم جديد لإضافته واعتماده:');
                      if (newCat && newCat.trim()) {
                        const clean = newCat.trim();
                        await addCategory(clean);
                        handleFormCategoryChange(clean);
                      }
                    }}
                    className="text-[11px] font-bold text-amber-500 hover:text-amber-400 flex items-center gap-1 hover:underline"
                  >
                ...
                {isCatDropdownOpen && (
                  <div className="absolute top-full right-0 left-0 mt-1 bg-stone-950 border border-stone-800 rounded-xl shadow-xl z-20 max-h-48 overflow-y-auto">
                    {formData.category?.trim() && !categories.includes(formData.category.trim()) && (
                      <button
                        ...
                        className="w-full text-right px-3 py-2 text-xs bg-amber-500/10 text-amber-400 border-b border-stone-800 hover:bg-amber-500/20 font-bold flex items-center justify-between"
                      >
                        <span>+ اعتماد "{formData.category.trim()}" كقسم جديد في النظام</span>
                      </button>
                    )}
